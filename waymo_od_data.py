...
def maybe_download_and_extract(data_dir):
    ...
    if not os.path.exists(train_dir):
        train_url = 'http://image-net.org/small/train_64x64.tar' # 4GB
        filepath = os.path.join(data_dir, 'train_64x64.tar')
        fetch(train_url, filepath)
        print('unpacking the tar file', filepath)
        tarfile.open(filepath, 'r').extractall(data_dir) # creates the train_64x64 folder

    test_dir = os.path.join(data_dir, 'valid_64x64')
    if not os.path.exists(test_dir):
        test_url = 'http://image-net.org/small/valid_64x64.tar' # 154MB
        filepath = os.path.join(data_dir, 'valid_64x64.tar')
        fetch(test_url, filepath)
        print('unpacking the tar file', filepath)
        tarfile.open(filepath, 'r').extractall(data_dir) # creates the valid_64x64 folder

def maybe_preprocess(data_dir):

    npz_file = os.path.join(data_dir, 'av_96_64.npz')
    ...
    for f in os.listdir(train_dir):
        if f.endswith('.jpg'):
            ...
            trainx.append(imread(filepath).reshape((1,64,96,3)))
            i += 1
    # i = 0
    # for f in os.listdir('/home/mkoren/scratch/Research/cs236project/data/av_96_64/'):
    #     if f.endswith('.jpg'):
    #         filepath = os.path.join(train_dir, f)
    #         trainx.append(imread(filepath)[:, 16:80, :].reshape((1,64,64,3)))
    #         i += 1
    #     if i > 1000:
    #         break
    ...
    for f in os.listdir(test_dir):
        if f.endswith('.jpg'):
            ...
            testx.append(imread(filepath).reshape((1,64,96,3)))
...
def load(data_dir, subset='train'):
    ...
    imagenet_data = np.load(os.path.join(data_dir,'av_96_64.npz'))
...
class DataLoader(object):
    ...
    def __init__(self, data_dir, subset, batch_size, rng=None, shuffle=False, **kwargs):
        ...
        self.data = load(os.path.join(data_dir,'av_96_64'), subset=subset)
